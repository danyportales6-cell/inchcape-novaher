...
const PARTE7 = () => {
  ...
  const cardsData = [
    {
      ...
      longDesc: "Ellos se encargaran de tu caso y tomaran medidas dentro del equipo. Acudir cuando el problema es reciente y necesitas una solución rapida. IMPORTANTE: Si el problema es tu superior, puedes dirigirte a otra área para poder solucionar tu situación.",
    },
    {
      id: 1,
      ...
      longDesc: "Te brindaran orientación y apoyo en el caso,tambien a gestionar las investigaciónes internas de forma confidencial. Acudir cuando necesitas acompañamiento formal y no se resolvio con tu superior.",
    },
    {
      id: 2,
      title: "ÁREA LEGAL",
      icon: "/legal.png",
      shortDesc: "Velamos por el cumplimiento de la ley y ética dentro de la empresa.",
      longDesc: "Te apoyaran con averiguar si hubo incumplimientos legales y te asesoraran sobre los derechos, apoyando en tu caso formalmente. Acudir cuando tu problema es de forma persistente y vulnera tus derechos..",
    },
    {
      id: 3,
      title: "ÁREA DE CUMPLIMIENTO",
      icon: "/cumplimiento.png",
      shortDesc: "Supervisa que se respenten las normas internas y éticas.",
      longDesc: "Recibiran las denuncias de forma anónima e investigara de forma parcial,asegurandose que se cumpla las politicas de la empresa. Acudir cuando buscas confidencialidad en tu caso y cuando involucra varias áreas, todo se resolvera bajo los criterios éticos dentro de la empresa..",
    },
  ];
      ...
      <div className="orientacion-container">
        ...
        <header className="orientacion-header animate-fade-in-down">
          ...
          <p className="orientacion-subtitle">
            Nunca dude en hacer preguntas, plantear inquietudes o buscar la asesoría que necesita. 
            Inchcape no tolerará ninguna represalia o discriminación contra quienes planteen una inquietud.
          </p>
        </header>
  ...
  <h3 className="card-title-front">{card.title}</h3>
</div>
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-content animate-pop-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-header">
              {typeof modalContent.icon === 'string' && modalContent.icon.length < 3 ? (
                <span className="modal-icon-emoji">{modalContent.icon}</span>
              ) : (
                <img src={modalContent.icon} alt={modalContent.title} className="modal-icon-img" />
              )}
            ...
            <div className="modal-footer">
              <button className="modal-action-btn" onClick={closeModal}>Entendido</button>
            </div>
          </div>
        </div>
      )}
